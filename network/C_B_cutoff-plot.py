...
def plot_c_B(c_B, figname):
    fig, ax = plt.subplots(figsize=(30,10))

    label = figname[20:-4].upper()
    cb, = ax.plot(c_B[:,0], c_B[:,1], label=f"{label}ngs") # Label: 3-Angs
    ss_range = {
        "ANK-H1": (4, 13),
        "ANK-H2": (17, 27),
        "ANK-H3": (46, 53),
    ...
    plt.legend(fontsize="xx-large", handles=[cb])
    fig.savefig(figname, dpi=100)

if __name__ == "__main__":
    pickle_list = [ "C_B_network-cutoff-3-a.pkl",
                    "C_B_network-cutoff-4-a.pkl",
                    "C_B_network-cutoff-5-a.pkl",
                    "C_B_network-cutoff-6-a.pkl",
                    "C_B_network-cutoff-7-allatom.pkl",
                    "C_B_network-cutoff-7-a.pkl"]
