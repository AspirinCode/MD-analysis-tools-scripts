#!/usr/bin/env python
docstring='''
reindex_pdb.py increment infile.pdb outfile.pdb
...
'''
# Script originally from https://zhanglab.ccmb.med.umich.edu/reindex_pdb/reindex_pdb.py
# Yang Zhang lab at UMichgan

# Functions: renumber the residue id with the increment specified
#            name the CBD residues correctly
#            add chain IDs correctly based on residue id order;
#            Orginall editted for processing pdb files written from DCD in VMD for TRPV2.
...
def reindex_pdb_autochain(startindex,infile,clean=True):
    '''parse PDB file "infile", reindex it according to start index or
    sequence file "startindex", and return the text of renumbered PDB
    '''
    fp=open(infile,'r')
    ...
    if os.path.isfile(startindex):
        ...
        PDBtxt_reindex=reindex_pdb_by_index_autochain(startindex,PDBtxt)
    return PDBtxt_reindex

def reindex_pdb_by_index_autochain(startindex=1,PDBtxt=''):
    '''
    reindex residue number of PDB format text
    ...
    for line in PDBtxt.splitlines():
        if len(line)<27 or (not line.startswith("ATOM  ") and \
            ...
            PDBtxt_reindex+=line+'\n'
            continue
        elif line[17:20] in hetres:
            hetID = line[75]
            PDBtxt_reindex+=line[:16]+' '+line[17:21]+hetID+line[22:]+'\n'

        else:
            old_resid = int(line[22:27])
            if old_resid < prev_resid: # a new chain
                chain_idx += 1

            prev_resid = old_resid
            chainID = chain_list[chain_idx]

            new_resid = old_resid + startindex
            resSeq_new=str(new_resid)
            resSeq_new=' '*(4-len(resSeq_new))+resSeq_new+' '

            PDBtxt_reindex+=line[:16]+' '+line[17:21]+chainID+resSeq_new+line[27:]+'\n'
...
if __name__=="__main__":
    ...
    PDBtxt_reindex=reindex_pdb_autochain(argv[0],argv[1],clean)
